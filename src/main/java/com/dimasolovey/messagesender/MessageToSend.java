...
public class MessageToSend {
    ...
    private static Map<String,JsonObject> dataPacketsMap = new HashMap<String, JsonObject>();
    private static List<DataPacket> listOfDataPackets = new ArrayList<DataPacket>();
    ...
    public static synchronized String getMessageFromJsonFormat (String jsonMessage, long timesatmp) {
        jsonParser = new JsonParser();
        Object object = jsonParser.parse(jsonMessage);
        JsonObject jsonObject = (JsonObject) object;
        imei = new IMEI((JsonObject) jsonObject.get("imei"));
        JsonArray dataPacketsAll = jsonObject.get("dataPackets").getAsJsonArray().get(0).getAsJsonArray();
        JsonObject firstElemOfArray = dataPacketsAll.get(1).getAsJsonObject();
        JsonArray log = firstElemOfArray.getAsJsonArray("log");
        if (log.size() == 0) {
            return "";
        }
        JsonObject packet = log.get(0).getAsJsonObject();
        JsonArray dataPackets = packet.get("dataPackets").getAsJsonArray();
        ...
        System.out.println(wiMetaConfiguration);
